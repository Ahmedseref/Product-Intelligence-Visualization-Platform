...
- **Key Features**:
    ...
    - **Proforma Invoice System**: A comprehensive proforma invoice generation system with customer management, product selection, inline editing, financial engine for custom calculations, PDF/Excel export, professional layout, **custom column manager** (text/number/formula columns with visibility, reorder, Discount % preset, custom row-total formula override via `shared/proformaFormula.ts`), and **versioning + duplication** (create new versions PI-XXXX-vN from any invoice, or duplicate as a brand-new PI-XXXX; version grouping in the list table with expandable sub-rows).
