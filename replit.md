...
- **Key Features**:
    ...
    - **Proforma Invoice System**: A comprehensive proforma invoice generation system with customer management, product selection, inline editing, financial engine for custom calculations, PDF/Excel export, professional layout, **custom column manager** (text/number/formula columns with visibility, reorder, Discount % preset, custom row-total formula override via `shared/proformaFormula.ts`), an optional **per-invoice Quantity formula** (the built-in Quantity column can be computed from a formula instead of manual entry; default stays manual) and a **user-editable Unit column** (per-row unit of measure that falls back to the catalog product unit), and **versioning + duplication** (create new versions PI-XXXX-vN from any invoice, or duplicate as a brand-new PI-XXXX; version grouping in the list table with expandable sub-rows).
    ...
    - **Product Qualification System**: Backend + UI for tagging products with substrate types, humidity tolerance, duty rating, finish type, and **layer position** (primer / base coat / intermediate / topcoat / standalone). Backed by `product_qualification_tags` and `qualification_vocabularies` (closed-list values, additive idempotent seed on startup). Exposed via `/api/qualification-vocabularies` and `/api/qualification-tags` (GET list/by-product, POST upsert, PATCH update, POST `/auto-infer`, POST `/auto-save-batch`). The deterministic `qualificationEngine.ts` infers all five fields from product name + description + taxonomy path; layer position drives a substrate override (base coat/intermediate → "Over Primer"; topcoat → "Over Base Coat" or "Over Primer" if name says so). UI lives in `SystemBuilderQualification.tsx` with main-table + auto-qualify review modal, both featuring sort/filter toolbars, quick-filter pills, conditional rendering driven by Layer Position (hides Finish for primers, hides Humidity for non-substrate-touching layers, locks Substrate to fixed pills for layered coats), and live taxonomy refresh.
