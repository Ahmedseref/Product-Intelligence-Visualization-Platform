...
export const suppliers = pgTable("suppliers", {
  ...
  mobile2: varchar("mobile_2", { length: 100 }),
  pendingPayment: text("pending_payment"),
  paidAmount: real("paid_amount"),
  invoiceValue: real("invoice_value"),
  reminder: timestamp("reminder"),
  updates: text("updates"),
  brand: jsonb("brand").$type<string[]>().default([]),
  product: jsonb("product").$type<string[]>().default([]),
  result: jsonb("result").$type<string[]>().default([]),
  filesMedia: jsonb("files_media").$type<{ name: string; url: string }[]>().default([]),
  tasksRelation: jsonb("tasks_relation").$type<string[]>().default([]),
  dailyTasksConnector: jsonb("daily_tasks_connector").$type<string[]>().default([]),
  relatedDocs: jsonb("related_docs").$type<string[]>().default([]),
  docsRelation: jsonb("docs_relation").$type<string[]>().default([]),
  // Full raw Notion properties payload from the last pull, kept for
  // complete fidelity (including any property not individually mapped above).
  notionRawProperties: jsonb("notion_raw_properties"),
