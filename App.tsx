...
const App: React.FC = () => {
  ...
  const [addProductMode, setAddProductMode] = useState<'single' | 'paste'>('single');
  ...
  const syncWithDatabase = useCallback(async () => {
    ...
    try {
      ...
      setProducts(productsData.map(p => ({
        ...
        productType: (p.productType as any) || 'standalone',
        category: p.category || '',
        sector: p.sector || '',
            ...
            {viewMode === 'add-product' && (
              <div className="space-y-6">
                <div className="flex items-center gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                  <span className="text-sm font-semibold text-slate-600">Add Mode:</span>
                  <div className="flex bg-slate-100 rounded-xl p-1">
                    ...
                    <button
                      onClick={() => setAddProductMode('paste')}
                      className={`px-4 py-2 text-sm font-semibold rounded-lg transition-all ${
                        addProductMode === 'paste'
                          ? 'bg-white text-blue-600 shadow-sm'
                          : 'text-slate-500 hover:text-slate-700'
                      }`}
                    >
